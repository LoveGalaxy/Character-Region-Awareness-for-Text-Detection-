...
def create_affine_boxes(boxes):
    ...
    for boxes_1, boxes_2 in zip(boxes[:-1], boxes[1:]):
        ...
        points_x1, points_y1 = triangle_center([center_x1, center_y1, x1, y1, x3, y3])
        points_x2, points_y2 = triangle_center([center_x1, center_y1, x2, y2, x4, y4])
        center_x2, center_y2, x1, y1, x3, y3, x2, y2, x4, y4 = box_center(boxes_2)
        points_x3, points_y3 = triangle_center([center_x2, center_y2, x1, y1, x3, y3])
        points_x4, points_y4 = triangle_center([center_x2, center_y2, x2, y2, x4, y4])
        affine_boxes.append([points_x1, points_y1, points_x2, points_y2, points_x3, points_y3, points_x4, points_y4])
    return affine_boxes
...
def find_min_rectangle(points):
    ...
    return np.float32([[lt_x, lt_y], [lt_x, rd_y], [rd_x, rd_y], [rd_x, lt_y]]), rd_x - lt_x, rd_y - lt_y
...
if __name__ == "__main__":
    boxes = [[0, 0, 20, 10], [12, 0, 20, 10]] #, [2, 1, 4, 3], [1, 3, 4, 5], [5, 2, 5, 4], [3, 4, 4, 6]]
    ...
    print(gaussian)
    pts = np.float32([[a_boxes[0][0], a_boxes[0][1]], [a_boxes[0][2], a_boxes[0][3]], [a_boxes[0][4], a_boxes[0][5]], [a_boxes[0][6], a_boxes[0][7]]])
